 ...
 */
@Tag(name = "internalOrder")

@Controller("/api/v1/internalOrder")
public class InternalOrder {
    private static final Logger LOG = LoggerFactory.getLogger(InternalOrder.class);
    ...
    @Post("/")
    public void receive(@Body InternalOrderMessage update) {
        try {
            bus.connect();
            String reply = bus.talkSync(exchangeName, "store.internal-order", mapper.writeValueAsString(update));
            if (reply == null) {
