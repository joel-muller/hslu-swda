...
public final class ServiceTemplate implements AutoCloseable {
    ...
    private void receiveLogs() throws IOException {
        LOG.debug("Starting listening for messages with routing [{}]", Routes.LOGS_NEW);
        bus.listenFor(exchangeName, "ServiceTemplate <- " + Routes.LOGS_NEW, Routes.LOGS_NEW, new LogReceiver(exchangeName, bus));
    }
